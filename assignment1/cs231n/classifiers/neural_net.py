...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        correct_class_scores = exp_scores[np.arange(N), y].reshape(N, 1)
        sum_class_scores = np.sum(exp_scores, axis=1, keepdims=True)

        data_loss = -np.log(correct_class_scores / sum_class_scores).mean()
        ...
        y_table = y_table == y.reshape(N, 1)
        d_scores = (exp_scores / sum_class_scores) - y_table
        grads['W2'] = a1.T.dot(d_scores)
        grads['W2'] /= N
        grads['b2'] = np.mean(d_scores, axis=0)

        # Backprop from the hidden layer
